...
// --- UNPROTECTED: GET all jobseeker profiles for the directory ---
// **IMPORTANT**: This route must be UNPROTECTED in your main server configuration.
router.get('/jobseekers', async (req, res) => {
    try {
        // Only fetch users who have completed essential profile details (e.g., have a profile_uuid)
        // Select only public, relevant fields
        ...
        const jobseekers = rows.map(user => {
            ...
            return {
                ...user,
                // These fields are already JSON strings in the database, parse them for the frontend
